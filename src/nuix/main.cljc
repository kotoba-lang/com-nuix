(ns nuix.main
  "Kotodama WASM entrypoint for the Nuix clean-room actor (L4) — Clojure port.
  ...
  facts (`nuix.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Matter"       :plural "matters"       :id-prefix "nuix_mat"
    :fields [:clientId :name :status :openedAt]   :required [:name :status]
    :coerce {}                       :refs {}}
   {:entity "Document"     :plural "documents"     :id-prefix "nuix_doc"
...
(defn emit-facts
  "EAVT facts for one record: {\"nuix.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nuix-compat" :tier tier :entities entities} 200])
